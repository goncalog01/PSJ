...
public:
  ...
  // spp added for anti-aliasing
	Light(Vector& pos, Color& col, int spp) : position(pos), color(col), spp(spp)
	{
		width = 1;
		length = 1;
		current = 0;
		std::mt19937 gen(rd());
		light_samples = std::vector<int>(spp);
		for (int i = 0; i < spp; i++) {
			light_samples[i] = i;
		}
	}

	Vector getLightSample();
	
	Vector position;
	Color color;
	int width, length, spp, current;
	std::vector<int> light_samples;

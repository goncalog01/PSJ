...
bool isPointInShadow(Ray shadow_ray) {
	if (Accel_Struct == NONE) {
		float dist;
		double length = shadow_ray.direction.length();
		shadow_ray.direction.normalize();
		for (int j = 0; j < scene->getNumObjects(); j++) {
			auto obj = scene->getObject(j);
			if (obj && obj->intercepts(shadow_ray, dist) && dist < length) {
				return true;
			}
		}
		return false;
		...
		return grid_ptr->Traverse(shadow_ray);
	}
	else if (Accel_Struct == BVH_ACC) {
		return bvh_ptr->Traverse(shadow_ray);
	}
}

Color rayTracing(Ray ray, int depth, float ior_1)  //index of refraction of medium 1 where the ray is travelling
{
	Color color(0.0f, 0.0f, 0.0f), reflection_color(0.0f, 0.0f, 0.0f), refraction_color;
	float current_dist, closest_dist = FLT_MAX, ni, nt;
  ...
  // closest intersection
	if (Accel_Struct == NONE) {
		for (int i = 0; i < scene->getNumObjects(); i++) {
			...
			if (obj && obj->intercepts(ray, current_dist) && current_dist < closest_dist) {
  ...
  // grid intersection
		...
		hit = grid_ptr->Traverse(ray, &closest_obj, hit_point);
	}
  // bvh intersection
	else if (Accel_Struct == BVH_ACC) {
		hit = bvh_ptr->Traverse(ray, &closest_obj, hit_point);
	}

  // no hit: paint with bg
	if (!hit) {
		return scene->GetSkyBoxFlg() ? scene->GetSkyboxColor(ray) : scene->GetBackgroundColor();
	}
	
	normal = closest_obj->getNormal(hit_point);
	if (-ray.direction * normal < 0) { // hit inside
		normal = -normal;
		ni = closest_obj->GetMaterial()->GetRefrIndex();
		nt = 1.0;
	}
	else { // hit outside
		ni = ior_1;
		nt = closest_obj->GetMaterial()->GetRefrIndex();
	}
	for (int i = 0; i < scene->getNumLights(); i++) {
		Light* light = scene->getLight(i);
    // anti-aliasing
		if (scene->GetSamplesPerPixel() != 0) {
			Vector light_sample = light->getLightSample();
			Vector l = light_sample - hit_point;
			Ray shadow_ray(hit_point + normal * EPSILON, l);
			l.normalize();
			if (l * normal > 0 && !isPointInShadow(shadow_ray)) {
				Vector h = l - ray.direction;
				h.normalize();
				color += closest_obj->GetMaterial()->GetDiffColor() * closest_obj->GetMaterial()->GetDiffuse() * (l * normal) + closest_obj->GetMaterial()->GetSpecColor() * closest_obj->GetMaterial()->GetSpecular() * pow((h * normal), closest_obj->GetMaterial()->GetShine());
			}
		}
    // no anti-aliasing
		else {
			float sqrt_n_lights = sqrt(N_LIGHTS);
			for (int p = 0; p < sqrt_n_lights; p++) {
				for (int q = 0; q < sqrt_n_lights; q++) {
					Vector l(light->position.x + (p + rand_float()) * (light->width / sqrt_n_lights), light->position.y, light->position.z + (q + rand_float()) * (light->length / sqrt_n_lights));
					Ray shadow_ray(hit_point + normal * EPSILON, l);
					l.normalize();
					if (l * normal > 0 && !isPointInShadow(shadow_ray)) {
						Vector h = l - ray.direction;
						h.normalize();
						color += (closest_obj->GetMaterial()->GetDiffColor() * closest_obj->GetMaterial()->GetDiffuse() * (l * normal) + closest_obj->GetMaterial()->GetSpecColor() * closest_obj->GetMaterial()->GetSpecular() * pow((h * normal), closest_obj->GetMaterial()->GetShine())) / (float) N_LIGHTS;
					}
				}
			}
		}
	...
	if (closest_obj->GetMaterial()->GetReflection() > 0) { // reflective
		Vector reverse_ray_dir = -ray.direction;
		Vector reflection_ray_dir = normal * (2 * (normal * reverse_ray_dir)) - reverse_ray_dir;
    // fuzzy reflections
		reflection_ray_dir = reflection_ray_dir + rnd_unit_sphere() * ROUGHNESS;
		reflection_ray_dir.normalize();
		Ray reflection_ray(hit_point + normal * EPSILON, reflection_ray_dir);
		reflection_color = closest_obj->GetMaterial()->GetSpecColor() * rayTracing(reflection_ray, depth + 1, ior_1);
	}

	if (closest_obj->GetMaterial()->GetTransmittance() > 0) { // dielectric
		Vector reverse_ray_dir = -ray.direction;
		Vector vt = normal * (normal * reverse_ray_dir) - reverse_ray_dir;
		float sin_theta_i = vt.norm();
		vt.normalize();
		float sin_theta_t = ni / nt * sin_theta_i;
		if (sin_theta_t <= 1.0f) { // reflection and refraction
			float cos_theta_t = sqrt(1 - pow(sin_theta_t, 2));

			// Schlick approximation
			float r0 = pow((ni - nt) / (ni + nt), 2);
			float cos_theta = ni > nt ? cos_theta_t : normal * reverse_ray_dir;
			float kr = r0 + (1 - r0) * pow(1 - cos_theta, 5);

			Vector refracted_ray_dir = vt * sin_theta_t + (-normal) * cos_theta_t;
			refracted_ray_dir.normalize();
			Ray refracted_ray(hit_point - normal * EPSILON, refracted_ray_dir);
			refraction_color = rayTracing(refracted_ray, depth + 1, nt);

			color += reflection_color * kr + refraction_color * (1 - kr);
		}
		else { // total reflection
			color += reflection_color * closest_obj->GetMaterial()->GetReflection();
		}
	}
	else { // only reflective
		color += reflection_color * closest_obj->GetMaterial()->GetReflection();
	}
...
void renderScene()
{
	...
	Color color;
	double sqrt_spp = sqrt(scene->GetSamplesPerPixel());
	...
	for (int y = 0; y < RES_Y; y++)
	{
		for (int x = 0; x < RES_X; x++)
		{
			...
			if (scene->GetSamplesPerPixel() == 0) { // no anti-aliasing no depth of field
				Vector pixel;
				pixel.x = x + 0.5f;
				pixel.y = y + 0.5f;
			...
			else if (scene->GetCamera()->GetAperture() == 0) { // anti-aliasing no depth of field
				for (int p = 0; p < sqrt_spp; p++)
				{
					for (int q = 0; q < sqrt_spp; q++)
					{
						Vector pixel;  //viewport coordinates
						pixel.x = x + (p + rand_float()) / sqrt_spp;
						pixel.y = y + (q + rand_float()) / sqrt_spp;
				...
				color /= scene->GetSamplesPerPixel();
			}
			else { // anti-aliasing depth of field
				for (int p = 0; p < sqrt_spp; p++)
				{
					for (int q = 0; q < sqrt_spp; q++)
					{
